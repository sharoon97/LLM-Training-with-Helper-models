"""Train the composite model after importing pretrained helper weights."""
...
def main():
    ...
    parser.add_argument("--helper-checkpoint", required=True)
    ...
    from common_functionsv13 import AdamWOpt, NewTransformerLM, import_phrase_weights, token_batch_stream, trainingSmallLM
    ...
    newtransLM = import_phrase_weights(newtransLM, args.helper_checkpoint, device, freeze_phrase=args.freeze_helper, trusted_checkpoint=True)
    ...
    output_file_name = os.path.join(args.output_dir, "checkpointPretrainedHelper")
    ...
    print("training has finished. Check if the checkpointPretrainedHelperFinal.pt is saved and then terminate this cell")
    torch.save({"model": copy.deepcopy(newtransLM.state_dict()), "optimizer": copy.deepcopy(AdamWOptx.state_dict()), "loss_history": list(loss_history)}, os.path.join(args.output_dir, "checkpointPretrainedHelperFinal.pt"))
